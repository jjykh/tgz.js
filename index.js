...
var TarGz = module.exports = function(zoptions, toptions) {
  ...
  this._options = {
    zlib: zoptions ? JSON.parse(JSON.stringify(zoptions)) : {},
    tar: toptions ? JSON.parse(JSON.stringify(toptions)) : {}
  };
};
 ...
 */
TarGz.prototype.createReadStream = function(directory, filter) {

  // Create all needed streams
  var stream1 = fstream.Reader({path: directory, filter: filter});
 ...
 */
TarGz.prototype.compress = function(source, destination, cb) {
  // Handle callbacks
  function ok() {
    if (cb)
        cb();
 ...
 * @return {Promise} An promise that will be fulfilled when the job is done
 */
TarGz.prototype.extract = function(source, destination, cb) {
  // Handle callbacks
  function ok() {
    if (cb)
        cb();
